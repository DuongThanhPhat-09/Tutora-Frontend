...
const StudentLessonDetail = () => {
    ...
    if (loading) {
        return <div className={s.page}><div className={s.loadingCenter}><Spin size="large" /></div></div>;
    }
            ...
            <div className={s.page}>
                ...
                <div className={s.mainContent}>
                    <div className={s.emptyState}>
                        <div className={s.emptyStateText}>Không tìm thấy buổi học</div>
                    </div>
                </div>
            </div>
        );
    ...
    return (
        <div className={s.page}>
            ...
            <div className={s.topBar}>
                <div className={s.topBarLeft}>
                    <h1 className={s.pageTitle}>Chi tiết buổi học</h1>
                    <p className={s.pageSubtitle}>Lesson #{lesson.lessonId} • Booking #{lesson.bookingId}</p>
                </div>
            </div>
            ...
            <div className={s.mainContent} style={{ maxWidth: '900px' }}>
                ...
                {/* Detail Info Panel */}
                <div className={s.detailPanel}>
                    <h2 className={s.sectionTitle}>Chi tiết buổi học</h2>
                            ...
                            <div className={s.detailItem}>
                                ...
                                <div>
                                    ...
                                    <a href={lesson.meetingLink} target="_blank" rel="noreferrer"
                                        style={{ fontSize: '14px', color: '#631b1b', fontWeight: 500 }}>
                ...
                {(lesson as any).report && (
                    <div className={s.detailPanel}>
                        <h2 className={s.sectionTitle}>Báo cáo gia sư</h2>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', fontSize: '14px', color: '#1a2238' }}>
                            {(lesson as any).report.contentCovered && (
                                <div><span style={{ color: '#737373' }}>Nội dung đã dạy:</span> {(lesson as any).report.contentCovered}</div>
                            )}
                            {(lesson as any).report.homeworkAssigned && (
                                <div><span style={{ color: '#737373' }}>Bài tập giao:</span> {(lesson as any).report.homeworkAssigned}</div>
                            )}
                            {(lesson as any).report.studentPerformanceRating != null && (
                                <div><span style={{ color: '#737373' }}>Đánh giá:</span> {(lesson as any).report.studentPerformanceRating}/5</div>
                            )}
                        </div>
                    </div>
                )}
